                        ...
                        <table class="table table-bordered">
                            ...
                            @foreach($categories as $category)
                                <tr>
                                    ...
                                    <td>
                                        <form method="post" action="{{ route('backend.video.category.delete', $category->id) }}">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <button type="button" class="mg2 btn-delete btn btn-danger pull-right">
                                                <i class="fa fa-trash-o"></i>
                                            </button>
                                        </form>
                                        <a class="mg2 btn btn-primary pull-right" href="{{ route('backend.video.category.edit', $category->id) }}"><i class="fa fa-pencil-square-o"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
